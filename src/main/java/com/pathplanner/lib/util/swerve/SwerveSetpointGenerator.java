  ...
  public SwerveSetpoint generateSetpoint(
    ...
    for (int m = 0; m < config.numModules; m++) {
      ...
      if (!epsilonEquals(0, moduleForceVec.getNorm())) {
        Rotation2d angleToModule = config.moduleLocations[m].getAngle().get();
        Rotation2d theta = moduleForceVec.getAngle().get().minus(angleToModule);
    ...
    for (int m = 0; m < config.numModules; m++) {
      ...
      double appliedForce =
          wheelForceDist > 1e-6
              ? wheelForceDist * wheelForces[m].getAngle().get().minus(retStates[m].angle).getCos()
              : 0.0;
