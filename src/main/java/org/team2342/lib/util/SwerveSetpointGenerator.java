...
/**
 * Modification of 254's setpoint generator
 *
...
public class SwerveSetpointGenerator {
  ...
  public SwerveSetpointGenerator(SwerveDriveKinematics kinematics) {
    ...
    this.moduleTranslations = kinematics.getModules();
  }
  ...
  public SwerveSetpoint generateSetpoint(
    ...
    if (epsilonEquals(desiredState, new ChassisVelocities())) {
      need_to_steer = false;
    ...
    double[] prev_vx = new double[moduleTranslations.length];
    double[] prev_vy = new double[moduleTranslations.length];
    Rotation2d[] prev_heading = new Rotation2d[moduleTranslations.length];
    double[] desired_vx = new double[moduleTranslations.length];
    double[] desired_vy = new double[moduleTranslations.length];
    Rotation2d[] desired_heading = new Rotation2d[moduleTranslations.length];
    ...
    for (int i = 0; i < moduleTranslations.length; ++i) {
      ...
      if (prevSetpoint.moduleStates()[i].velocity < 0.0) {
        prev_heading[i] = prev_heading[i].rotateBy(Rotation2d.PI);
      }
      ...
      if (desiredModuleState[i].velocity < 0.0) {
        desired_heading[i] = desired_heading[i].rotateBy(Rotation2d.PI);
      }
    ...
    if (all_modules_should_flip
        && !epsilonEquals(prevSetpoint.chassisSpeeds(), new ChassisVelocities())
        && !epsilonEquals(desiredState, new ChassisVelocities())) {
    ...
    List<Optional<Rotation2d>> overrideSteering = new ArrayList<>(moduleTranslations.length);
    ...
    retSpeeds = retSpeeds.discretize(0.02);
    ...
    for (int i = 0; i < moduleTranslations.length; ++i) {
      ...
      if (flipHeading(deltaRotation)) {
        retStates[i].angle = retStates[i].angle.rotateBy(Rotation2d.PI);
        retStates[i].velocity *= -1.0;
      }
    }
